...
print("\n\n\n******* BOOTING LIBRARY TERMINAL SYSTEM *******\n")

print('Todays Date:' + dateToday )
...
#Use CSV reader to create a book dictionary and a book list. 
try:
    print("\nLoading CSV data...")
    with open ("books.csv", mode ='r', encoding = "utf-8-sig") as file:
        ...
        for row in csvFile:         
            if rowCount == 0:
                ...
                booksList += [row[0]] 
                booksDict[row[0]] = {name1:formatName(row[1]),name2:formatName(row[2]),name3:formatName(row[3]),name4:formatName(row[4]), name5:row[5], 'Available':'Yes', 'Reserved':'No', 'ReservedID':[]}
    ...
    with open ('bookloans.csv', mode = 'r', encoding = 'utf-8-sig') as file:
        ...
        for row in csvFile:
            if row[0] in booksList:
                ...
                loanDict[rowCount] = {'BookID':row[0], 'MemberID':row[1], 'Date of loan': formatDate("1900-01-01", row[2]), 'Data of return': dateOfReturn, 'Length of Borrow':lengthOfBorrow,}
                #print("CSV imported successfully...")
except:
    ...
    with open('members.csv', mode = 'r', encoding = 'utf-8-sig') as file:
        ...
        for row in csvFile:
            if rowCount == 0:
                ...
                memberDict[row[0]] = {name1:row[1], name2:row[2], 'Name': (row[1]+ " " + row[2]), name3:row[3], name4:row[4], name5:row[5]}
                rowCount +=1
except:
...
class Book(object):
    """ The Book class can contain methods that are related to the Book class. Other classes may inherit methods 
         from the book class.
        pre-Conditions:  The book class assumes there is a dictionary available called books that has been loaded 
                         into memory from books.json
                         The exact book title needs to be entered for the scan method to function correctly. the 
                         title should be entered as an argument in the scan method.
...
class Member(object):
    ...
    def scan(cls, name):
        ...
        if len(ID) >= 1:
            ...
            return print(name, "does not have an account. Please apply for membership or check member details")
    ...
    def apply(cls):
        ...
        members[member_no] = {'Firstname':first_name, 'Last Name':last_name, 'Gender':gender, 'Email':email , 'CardNumber':str(member_no) + "1"}
...
#Sanity check the class functionality
...
class Loan(Book, Member):
    ...
    def Borrow(cls, name, title):
        ...
        if Status == "Yes":
            ...
            if books[BookID]['Reserved'] == "No":
                ...
                print("")
                print("The loan ID for this book loan is", LoanID, "Please return the book using this ID.")
            ...
            elif books[BookID]["Reserved"] == "Yes" and MemberID in books[BookID]["ReservedID"] == MemberID:
                print("Unfortunately",title, "is not available at the moment. You have been put on the reserved list for this book.")
                
            else:
                print(title, "is not available at the moment, you can reserve this book if you like?")
    ...
    def Return(cls, name, title):
        ...
        if Status == "Yes":
            print("Error... The requested book return has failed. This book appears to already be available in the database???")
            
        elif Status == "No":
            books[BookID]['Available'] = "Yes"
            loans[LoanID]['Date of return'] = dateToday
            
            print(title, "has been successfully returned. Thank you for using the library", name,".")
...
if option == 3:
    ...
    if staff_option == 1:
        print("At the end of the working day please email the new_member_details.txt files to the printers.")
